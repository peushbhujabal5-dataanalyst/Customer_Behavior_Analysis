...
select item_purchased,avg(review_rating)rv_rating from customer
 group by item_purchased order by rv_rating desc limit 5;
...
select subscription_status,count(customer_id)total_customer ,round(avg(purchase_amount),2)avg_spend,
round(sum(purchase_amount),2)total_revenue from customer 
...
select subscription_status,count(customer_id)repeat_buyers from customer
